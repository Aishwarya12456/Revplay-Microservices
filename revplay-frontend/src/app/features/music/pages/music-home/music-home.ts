...
export class MusicHomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.currentArtistId = this.authService.getArtistId();
  ...
  deleteSong(songId: number) {
    ...
    this.musicService.deleteSong(songId).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
        this.songs = this.songs.filter(s => s.id !== songId);
        this.showToast('Song deleted successfully 🗑️');
